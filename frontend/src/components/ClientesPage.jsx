...
export default function ClientesPage() {
  ...
  const [expanded, setExpanded] = useState({}); // { [albaranId]: { loading, error, lineas } }
  ...
  const activeChips = [
    ...
    ...(sort !== 'az' ? [{ key: 'sort', label: `Orden: ${({ az:'A→Z', za:'Z→A', id_up:'ID↑', id_down:'ID↓' })[sort]}`, onRemove: () => setSort('az') }] : []),
  ];
  ...
  // Cargar líneas de un albarán bajo demanda
    ...
    <div className="p-6">
      ...
      {activeChips.length > 0 && (
        <div className="flex flex-wrap items-center gap-2 mb-4">
          ...
          <button className="text-sm text-gray-600 underline ml-2" onClick={clearAll}>
            Limpiar todo
          </button>
        </div>
      )}
      ...
      <ModalCenter isOpen={filtersOpen} onClose={() => setFiltersOpen(false)} maxWidth="max-w-lg">
          ...
          <div>
            ...
            <div className="flex items-center gap-3">
              <input
                type="number"
                value={idRange[0]}
                onChange={(e) => setIdRange([Number(e.target.value), idRange[1]])}
              ...
              <input
                type="number"
                value={idRange[1]}
                onChange={(e) => setIdRange([idRange[0], Number(e.target.value)])}
                className="w-32 border border-gray-300 rounded-lg px-3 py-2"
              />
            </div>
          </div>
        ...
        <div className="mt-8 flex items-center justify-between">
          ...
          <button onClick={() => setFiltersOpen(false)} className="px-4 py-2 rounded-xl bg-black text-white">
            Aplicar
          </button>
        </div>
      </ModalCenter>

      {/* Modal Detalle Cliente (centrado) con albaranes mejorados */}
      <ModalCenter isOpen={detailOpen} onClose={closeDetail} maxWidth="max-w-4xl">
        ...
        {!selected ? (
          ...
          <div className="space-y-6">
            ...
            {/* Resumen y listado de albaranes */}
