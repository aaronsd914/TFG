"""
test_resumen_semanal.py â€” Tests del mÃ³dulo de resumen semanal.

Cubre:
  - _get(): lectura de clave con y sin valor en la BD
  - _set(): creaciÃ³n y actualizaciÃ³n de clave en la BD
  - _eur(): formateo de nÃºmeros como euros
  - _build_html(): construcciÃ³n del HTML del resumen
...
# â”€â”€ _get â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
class TestGet:
    ...
    def test_devuelve_default_cuando_valor_vacio(self, db):
        db.add(ConfigDB(key="tienda_nombre", value=""))
        db.commit()
        assert _get(db, "tienda_nombre") == "FurniGest"

    def test_devuelve_valor_almacenado(self, db):
        db.add(ConfigDB(key="resumen_email_destino", value="a@b.com"))
        db.commit()
...
# â”€â”€ _set â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
class TestSet:
    def test_crea_nueva_fila(self, db):
        _set(db, "resumen_email_destino", "x@y.com")
        row = db.query(ConfigDB).filter_by(key="resumen_email_destino").first()
        assert row is not None
        assert row.value == "x@y.com"

    def test_actualiza_fila_existente(self, db):
        db.add(ConfigDB(key="tienda_nombre", value="viejo"))
        db.commit()
        _set(db, "tienda_nombre", "nuevo")
        row = db.query(ConfigDB).filter_by(key="tienda_nombre").first()
        assert row.value == "nuevo"


# â”€â”€ _eur â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
class TestEur:
...
# â”€â”€ _md_to_html â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
class TestMdToHtml:
    ...
    def test_saltos_de_linea_a_br(self):
        # Single newline â†’ space; double newline â†’ <br><br>
...
# â”€â”€ _build_html â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
class TestBuildHtml:
...
class TestRun:
    ...
    def test_omite_si_intervalo_no_ha_transcurrido(self, db):
        ...
        db.add(ConfigDB(key="resumen_email_destino", value="a@b.com"))
        db.add(ConfigDB(key="resumen_ultima_vez", value=ayer))
        db.add(ConfigDB(key="resumen_intervalo_dias", value="7"))
        db.commit()
        with patch("backend.app.utils.resumen_semanal.send_email_simple") as mock_email:
            _run(db)
        mock_email.assert_not_called()
    ...
    def test_ejecuta_cuando_intervalo_ha_transcurrido(self, db):
        ...
        db.add(ConfigDB(key="resumen_email_destino", value="a@b.com"))
        db.add(ConfigDB(key="resumen_ultima_vez", value=hace_diez))
        db.add(ConfigDB(key="resumen_intervalo_dias", value="7"))
        db.commit()
        with patch("backend.app.utils.resumen_semanal.send_email_simple") as mock_email, \
    ...
    def test_maneja_error_de_groq_y_sigue_enviando(self, db):
        ...
        db.commit()
        with patch("backend.app.utils.resumen_semanal.send_email_simple") as mock_email, \
             patch("backend.app.utils.resumen_semanal.groq_chat", side_effect=Exception("LLM caÃ­do")):
            _run(db)
        mock_email.assert_called_once()

    def test_ignora_fecha_invalida_en_ultima_vez(self, db):
        db.add(ConfigDB(key="resumen_email_destino", value="a@b.com"))
        db.add(ConfigDB(key="resumen_ultima_vez", value="not-a-date"))
        db.commit()
        with patch("backend.app.utils.resumen_semanal.send_email_simple") as mock_email, \
             patch("backend.app.utils.resumen_semanal.groq_chat", return_value=""):
            _run(db)
...
# â”€â”€ job_resumen_semanal â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
