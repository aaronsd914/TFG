...
Se ha aplicado una estrategia rigurosa de ingeniería del software: más de 850 pruebas automatizadas (unitarias, de integración y end-to-end), análisis estático con Ruff y ESLint, \emph{quality gate} con SonarCloud, un pipeline de integración y despliegue continuos con GitHub Actions, y contenerización con Docker. El sistema está desplegado en producción en Railway (backend) y Vercel (frontend).

\mbox{} \bigskip

\noindent \textbf{Palabras clave}: aplicación web, SPA, React, FastAPI, Python, REST, gestión empresarial, máquina de estados, integración continua, despliegue continuo, pruebas automatizadas, inteligencia artificial, LLM, analíticas de negocio, RFM, Market Basket Analysis.
...
Este capítulo introduce el contexto en el que surge FurniGest y delimita su alcance dentro del proyecto. Se compone de tres secciones: la primera presenta el contexto y la motivación que justifican el desarrollo de un sistema de gestión a medida; la segunda detalla el alcance funcional cubierto y lo que queda fuera del proyecto; y la tercera describe la estructura de la presente memoria.
...
En este capítulo se formaliza el problema que aborda FurniGest, se evalúan las alternativas existentes y se concretan los objetivos del proyecto, junto con la metodología de desarrollo aplicada y la planificación temporal seguida. El capítulo se divide en cinco secciones: descripción del problema, estudio de alternativas, objetivos del proyecto, metodología de desarrollo y planificación temporal.
...
A partir de la descripción del problema y del estudio de alternativas, esta sección concreta los objetivos que han guiado el desarrollo del sistema. Se distingue entre un \textbf{objetivo general}, que sintetiza el propósito global del proyecto, y un conjunto de \textbf{objetivos específicos} verificables que dan forma a las funcionalidades y a los criterios de aceptación de FurniGest.
...
\begin{table}[H]
  ...
  \begin{tabularx}{\textwidth}{lX}
    ...
    OBJ-05 & Incorporar cobro con tarjeta a través de Stripe Checkout con registro automático del pago como movimiento. \\
    ...
    OBJ-07 & Construir un motor de analíticas con segmentación RFM de clientes, análisis de cesta y predicción de ingresos (Holt). \\
...
Este capítulo recoge el análisis del sistema y las decisiones de diseño que sustentan la implementación de FurniGest. Se compone de cinco secciones: la especificación de requisitos funcionales y no funcionales, la arquitectura general del sistema, el modelo de datos relacional, el diseño de la API REST y la justificación de las decisiones tecnológicas adoptadas.
...
\begin{longtable}{lp{11cm}}
  ...
  RF-03 & Ciclo de vida del albarán: máquina de cinco estados (FIANZA $\to$ ALMACEN $\to$ RUTA $\to$ ENTREGADO; ENTREGADO $\leftrightarrow$ INCIDENCIA) con transiciones controladas. \\
...
Esta sección describe la arquitectura general que sustenta FurniGest. Se parte de un esquema cliente-servidor de tres capas y se desglosa la organización interna del backend en capas con sus patrones de diseño asociados, así como la estructura del frontend basada en una SPA con \emph{shell} persistente y gestión de estado mediante \emph{contextos} de React.
...
Cada tecnología se ha elegido de forma consciente. La tabla~\ref{tab:stack} recoge el \emph{stack} tecnológico con su justificación; la tabla~\ref{tab:alternativas_tech} las alternativas descartadas.
...
Este capítulo detalla cómo se ha llevado a la práctica el diseño descrito en el capítulo anterior. Se estructura en seis secciones: la organización del proyecto, la implementación del backend con FastAPI, los flujos de negocio principales, los algoritmos avanzados aplicados, la integración con un modelo de lenguaje y el desarrollo del frontend en React.

\section{Estructura del proyecto}

El repositorio se organiza en tres directorios principales: el del backend (FastAPI y SQLAlchemy), el del frontend (React SPA) y el de pruebas (unitarias, de integración y E2E). Los ficheros de configuración con secretos están excluidos del repositorio mediante el correspondiente fichero de exclusión.
...
\begin{figure}[H]
  ...
  \begin{tikzpicture}[
    ...
    %% ── Actores ─────────────────────────────────────────────
    \node[box, fill=blue!10]   (fe) at ( 0, 0) {Frontend\\(BancoPage)};
    \node[box, fill=green!10]  (be) at ( 6, 0) {Backend\\(FastAPI)};
    \node[box, fill=orange!10] (sa) at (12, 0) {Stripe\\API};
    \node[box, fill=orange!15] (sc) at (18, 0) {Stripe\\Checkout};

    %% ── Líneas de vida ──────────────────────────────────────
    \foreach \x in {0, 6, 12, 18}
      \draw[dashed, gray!55] (\x,-0.45) -- (\x,-9.2);

    %% ── Mensajes ────────────────────────────────────────────
    \draw[->] ( 0,-1.1) -- node[above,lbl]{1.~checkout}          ( 6,-1.1);
    \draw[->] ( 6,-2.1) -- node[above,lbl]{2.~Session.create}    (12,-2.1);
    \draw[->] (12,-3.1) -- node[above,lbl]{3.~session\_id}        ( 6,-3.1);
    \draw[->] ( 6,-4.1) -- node[above,lbl]{4.~redirect\_url}      ( 0,-4.1);
    \draw[->] ( 0,-5.1) -- node[above,lbl]{5.~redirect}           (18,-5.1);
    \draw[->] (18,-6.1) -- node[above,lbl]{6.~success\_url}       ( 0,-6.1);
    \draw[->] ( 0,-7.1) -- node[above,lbl]{7.~confirm}            ( 6,-7.1);
    \node[lbl, anchor=west] at (6.3,-8.1)
      {\textit{8.~Verificar pago $\to$ registrar movimiento INGRESO}};
  \end{tikzpicture}}
...
La implementación genera los pares de productos y cuenta co-ocurrencias en un solo recorrido. La \textbf{complejidad} es $O(m \cdot k^2)$ donde $m$ es el número de pedidos y $k$ el número medio de productos por pedido.
...
\begin{table}[H]
  ...
  \begin{tabular}{ll>{\raggedright\arraybackslash}p{6.5cm}}
    ...
    \midrule
    \texttt{/login}          & LoginPage           & Autenticación de usuario \\
...
Este capítulo constituye el núcleo de la ingeniería del software del proyecto y describe cómo se garantiza la calidad y la puesta en producción del sistema. Se organiza en nueve secciones: estrategia de pruebas, pruebas de integración del backend, pruebas unitarias del frontend, pruebas end-to-end, análisis estático de código, integración continua, contenerización con Docker, despliegue en producción y migraciones de base de datos con Alembic.
...
\begin{figure}[H]
  ...
  \begin{tikzpicture}
    ...
    \node at (3,0.9) {\textbf{Unitarias: 511}};
    \node at (3,2.6) {\textbf{Integración: 239}};
    \node at (3,4.0) {\textbf{E2E: 107}};

    \node[right] at (6.3,0.8) {\small Vitest};
    \node[right] at (6.3,2.5) {\small pytest + TestClient};
...
\begin{table}[H]
  ...
  \begin{tabular}{llcc}
    ...
    \midrule
    Unitario frontend      & Vitest 4          & 511  & $>$ 80\,\% \\
    Integración backend    & pytest 9          & 239  & $>$ 80\,\% \\
    End-to-end             & Selenium          & 107  & Flujos completos \\
    \midrule
    \textbf{Total}      &                & \textbf{857} & \\
    \bottomrule
  \end{tabular}
\end{table}


\section{Pruebas de integración del backend}
...
\begin{longtable}{l>{\centering\arraybackslash}p{1cm}>{\raggedright\arraybackslash}p{6cm}}
  \caption{Tests unitarios del frontend.} \label{tab:tests_frontend} \\
  \toprule
  ...
  Sidebar.test.jsx             & 13 & Enlaces nav, clases CSS activas, logout \\
  App.test.jsx                 &  7 & Renderizado root, layout, aside + main \\
  Dashboard.test.jsx           & 10 & KPIs, gráficos, datos vacíos, error de red \\
  AlbaranesPage.test.jsx       & 38 & CRUD, filtros estado, modal, PDF, errores API \\
  BancoPage.test.jsx           & 43 & Estado Stripe, checkout, confirmación de pago \\
  ClientesPage.test.jsx        & 33 & CRUD, búsqueda, modales, error de red \\
  NuevaVenta.test.jsx          & 39 & Flujo completo, búsqueda cliente, confirmación \\
  ProductosPage.test.jsx       & 28 & CRUD productos + proveedores, pestañas \\
  Tendencias.test.jsx          & 27 & Gráficos, chat IA, exportar PDF \\
  TransportePage.test.jsx      & 38 & Asignar camión, liquidar ruta \\
  IncidenciasPage.test.jsx     & 28 & Listar, crear, eliminar, estados \\
  i18n.test.jsx                & 14 & Detección idioma, traducciones, cambio de idioma \\
  api.test.js                  & 29 & Cliente API, construcción de peticiones, errores \\
  hooks.test.jsx               &  8 & Hooks de estado, efectos, limpieza \\
  http.test.js                 &  8 & Utilidades HTTP, URL base, cabeceras \\
  i18n.test.js                 &  9 & Configuración i18n, locale, pluralización \\
  ModalCenter.test.jsx         &  6 & Renderizado portal, cierre con backdrop \\
  UIComponents.test.jsx        & 34 & Botones, inputs, badges, modales, controles de formulario \\
  \midrule
...
La tabla~\ref{tab:tests_e2e} lista los 12 ficheros de tests E2E.
...
El proyecto utiliza un único \textbf{fichero de workflow de GitHub Actions} \cite{githubactions} que define siete \emph{jobs}:

\begin{table}[H]
  \centering
  \caption{Workflow de GitHub Actions.}
  \label{tab:workflows}
  \begin{tabularx}{\textwidth}{llX}
    \toprule
...
Este capítulo presenta los resultados obtenidos tras la implementación y el despliegue de FurniGest. Se divide en cuatro secciones: la evaluación heurística de usabilidad siguiendo las heurísticas de Nielsen, las métricas de calidad del software medidas por la suite de pruebas y SonarCloud, las capturas de pantalla del sistema en funcionamiento y las URLs públicas del entorno de producción.
...
\begin{table}[H]
  ...
  \begin{tabular}{lc}
    ...
    Bugs detectados             & 0 \\
    Vulnerabilidades            & 0 \\
    Tests de integración backend & 239 \\
    Tests unitarios frontend    & 511 \\
    Tests E2E (Selenium)        & 107 \\
    Tests totales               & 857 \\
    Endpoints REST              & 56 \\
    Entidades de dominio        & 11 \\
    Ficheros de workflow CI/CD  & 1 (7 jobs) \\
    \bottomrule
  \end{tabular}
\end{table}
...
Las siguientes figuras muestran las once vistas principales de FurniGest en su entorno de producción.
...
\begin{figure}[H]
  ...
  \caption{Dashboard: KPIs principales, gráfico de ingresos y distribución de estados.}
  \label{fig:cap_dashboard}
\end{figure}

\begin{figure}[H]
  \centering
  \includegraphics[width=0.85\textwidth]{{Nueva venta}.png}
  \caption{Nueva Venta: asistente paso a paso con búsqueda de cliente y adición de productos.}
  \label{fig:cap_nueva_venta}
\end{figure}

\begin{figure}[H]
  \centering
  \includegraphics[width=0.85\textwidth]{Clientes.png}
  \caption{Clientes: tabla con búsqueda y panel lateral de historial de pedidos.}
  \label{fig:cap_clientes}
\end{figure}

\begin{figure}[H]
  \centering
  \includegraphics[width=0.85\textwidth]{Albaranes.png}
  \caption{Albaranes: lista filtrable con control de estado inline.}
  \label{fig:cap_albaranes}
\end{figure}

\begin{figure}[H]
  \centering
  \includegraphics[width=0.85\textwidth]{Productos.png}
  \caption{Productos y proveedores: vista dividida.}
  \label{fig:cap_productos}
\end{figure}

\begin{figure}[H]
  \centering
  \includegraphics[width=0.85\textwidth]{Tendencias.png}
  \caption{Tendencias: gráficos analíticos, segmentación RFM, predicción y chat IA.}
  \label{fig:cap_tendencias}
\end{figure}

\begin{figure}[H]
  \centering
  \includegraphics[width=0.85\textwidth]{Transporte.png}
  \caption{Transporte: columna de almacén y columnas por camión.}
  \label{fig:cap_transporte}
\end{figure}

\begin{figure}[H]
  \centering
  \includegraphics[width=0.85\textwidth]{Banco.png}
  \caption{Banco: panel de pagos Stripe con historial de cobros.}
  \label{fig:cap_banco}
\end{figure}

\begin{figure}[H]
  \centering
  \includegraphics[width=0.85\textwidth]{Incidencias.png}
  \caption{Incidencias: gestión de incidencias de entrega.}
  \label{fig:cap_incidencias}
\end{figure}

\begin{figure}[H]
  \centering
  \includegraphics[width=0.85\textwidth]{{Configuración}.png}
  \caption{Personalización: selector de paleta, modo oscuro e idioma.}
  \label{fig:cap_personalizacion}
\end{figure}
...
\begin{table}[H]
  ...
  \begin{tabularx}{\textwidth}{lX}
    ...
    \midrule
    Frontend      & \url{https://furnigest.com} \\
...
Este capítulo cierra la memoria valorando el grado de cumplimiento de los objetivos planteados y proponiendo líneas de evolución para el sistema. Se compone de dos secciones: la primera recoge las conclusiones del trabajo realizado y la segunda enumera los trabajos futuros propuestos.
...
\begin{description}[nosep]
  ...
  \item[OBJ-05 (Stripe):] Cumplido. El flujo de pago con Stripe Checkout funciona con registro automático de ingresos e idempotencia.
  ...
  \item[OBJ-07 (Analíticas):] Cumplido. El motor implementa segmentación RFM, análisis de cesta y predicción de Holt, todo accesible desde la pantalla de tendencias.
  \item[OBJ-08 (Pruebas):] Cumplido. Más de 850 pruebas automatizadas cubren los tres niveles de la pirámide de pruebas.
  \item[OBJ-09 (CI/CD):] Cumplido. Un único workflow de GitHub Actions (con siete jobs) garantiza que cada cambio pasa por pruebas, linting, quality gate y despliegue.
...
A continuación se proponen las principales líneas de evolución para FurniGest, que permitirían ampliar su alcance funcional y reforzar su robustez en escenarios de uso más exigentes:

\begin{description}[nosep]
  \item[Roles y permisos granulares.] Diferenciar permisos de vendedor, repartidor y administrador con listas de control de acceso.
  \item[Gestión de stock e inventario.] Registrar entradas y salidas de almacén en tiempo real, con alertas de stock mínimo.
  \item[Aplicación móvil (PWA o React Native).] Acceso optimizado para repartidores en ruta con capacidades offline.
  \item[Caché Redis para analíticas.] Reducir la carga de consultas frecuentes al motor de analíticas.
  \item[WebSockets para notificaciones.] Actualizar el dashboard en tiempo real cuando se crea un pedido o cambia un estado.
  \item[Módulo de presupuestos.] Generar y enviar presupuestos previos al pedido formal.
  \item[Soporte multi-tienda.] Gestionar varias sucursales desde una única instancia.
  \item[Pruebas de carga (Locust).] Validar el rendimiento del sistema bajo concurrencia elevada.
  \item[Conciliación bancaria automática.] Integración con Open Banking PSD2 para detectar automáticamente cobros recibidos y conciliarlos con albaranes pendientes de pago.
\end{description}
...
Al arrancar por primera vez, \texttt{seed.py} genera: 5 proveedores, 200 productos, 150 clientes, 400 albaranes y los movimientos financieros asociados. El seed es idempotente: si ya hay datos, no se ejecuta.
