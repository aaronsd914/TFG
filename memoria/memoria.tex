...
\usepackage{quotchap}
\usepackage{tikz}
\usetikzlibrary{shapes.geometric,arrows.meta,positioning,fit,calc,backgrounds,automata}
...
This Bachelor's Thesis presents the design, implementation and deployment of \textbf{FurniGest}, a Management Information System (MIS) for a furniture store. The application centralises customer, product, supplier, delivery note, financial movement, transport logistics and business analytics management on a web platform accessible from any browser.

The backend was developed with \textbf{FastAPI} (Python 3.12), \textbf{SQLAlchemy 2} as ORM, and \textbf{PostgreSQL 16} as the database engine. The frontend is a single-page application built with \textbf{React 19}, \textbf{Vite 6} and \textbf{TailwindCSS 4}. Both layers communicate through an OpenAPI-documented REST API.

Advanced features include an analytics engine with RFM customer segmentation and Market Basket Analysis; an AI assistant powered by Llama-3 via Groq; revenue forecasting using Holt's double exponential smoothing; Stripe Checkout payment integration; automatic PDF generation with ReportLab; email delivery with attachments; and full internationalisation (ES/EN).

A rigorous software engineering strategy was applied throughout: over 540 automated tests (unit, integration and end-to-end), static analysis with Ruff and ESLint, a SonarCloud quality gate, a CI/CD pipeline with GitHub Actions for continuous integration and deployment, and Docker containerisation. The system is deployed in production on Railway (backend) and Vercel (frontend).
...
\begin{compactitem}
    ...
    \item Artificial intelligence (LLM)
    \item Data analytics (RFM, Market Basket)
\end{compactitem}
...
\begin{longtable}{lp{11cm}}
  ...
  RF-19 & Integración de pagos (Stripe): creación de sesiones de cobro mediante Stripe Checkout, confirmación de pagos y consulta del historial de cobros. \\
  \bottomrule
\end{longtable}
...
\begin{table}[H]
  ...
  \begin{tabular}{lll}
    ...
    Banca (Stripe) & 4 & Estado, crear sesión de cobro, confirmar pago, listar cobros \\
...
\begin{figure}[H]
  \centering
  \begin{tikzpicture}[
    step/.style={rectangle, draw, rounded corners, minimum width=2.2cm, minimum height=0.8cm, align=center, font=\small},
    >=Stealth
  ]
    \node[step, fill=gray!15]    (lint)    {Lint\\(Ruff + ESLint)};
    \node[step, fill=green!15, right=0.8cm of lint, yshift=0.6cm]   (tfe)     {Frontend\\tests};
    \node[step, fill=green!15, right=0.8cm of lint, yshift=-0.6cm]  (tbe)     {Backend\\tests};
    \node[step, fill=yellow!15, right=0.8cm of tfe, yshift=-0.6cm]  (sonar)   {SonarCloud\\gate};
    \node[step, fill=orange!15, right=0.8cm of sonar]  (e2e)     {E2E\\Selenium};
    \node[step, fill=purple!15, right=0.8cm of e2e]    (docker)  {Docker\\build};
    \node[step, fill=blue!15, right=0.8cm of docker]   (deploy)  {Deploy\\Railway+Vercel};
...
\begin{table}[H]
  ...
  \begin{tabularx}{\textwidth}{lX}
    ...
    Conciliación bancaria automática & Ampliar la integración con Stripe para detectar automáticamente cobros recibidos y conciliarlos con albaranes pendientes de pago. \\
    \bottomrule
  \end{tabularx}
\end{table}
...
\begin{longtable}{llp{6cm}}
  ...
  \midrule
  \multicolumn{3}{l}{\textbf{Banca (Stripe)}} \\
  GET    & /api/stripe/status                    & Estado de configuración \\
  POST   & /api/stripe/create-checkout-session   & Crear sesión de cobro \\
  GET    & /api/stripe/confirm                   & Confirmar pago completado \\
  GET    & /api/stripe/charges                   & Listar cobros \\
  \midrule
