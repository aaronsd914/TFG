...
def update_movement(
    movement_id: int, payload: MovementCreate, db: Annotated[Session, Depends(get_db)]
):
    return movimientos_service.update_movement(movement_id, payload, db)


@router.delete("/movimientos/delete/{movement_id}")
def delete_movement(movement_id: int, db: Annotated[Session, Depends(get_db)]):
